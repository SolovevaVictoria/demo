...
public class ExceptionHandlingAspect {
    @Pointcut("@annotation(ru.geekbrains.demo.aspect.RecoverException)")
    public void exceptionPointcut() {
    }

    @Around("exceptionPointcut()")
    public Object Exception(ProceedingJoinPoint joinPoint, RecoverException recoverException) throws Throwable {
        try {
            return joinPoint.proceed();
        } catch (RuntimeException exeption) {
            for (Class<? extends RuntimeException> nonRecoverable : recoverException.noRecoverFor()) {
                if (nonRecoverable.isInstance(exeption)) {
                    throw exeption;
                }
            }
